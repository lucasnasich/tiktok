...
export function cursorPromptForSlotDescription(spec: SlotSpec) {
  return [
    ...
    "- Además guardá en `inspirationSearchBrief` un párrafo corto (1 a 3 oraciones) que le diga al usuario qué tipo de referencia buscar en la biblioteca: mecanismo visual, si conviene cámara o pantalla, tono, ritmo, etc. Es una guía para elegir inspiración, no copy de la pieza.",
    "- No inventes features, pricing, clientes ni claims. Consultá el Mercantis Brain.",
    "",
    "Después de redactarlas, guardalas en el SlotSpecRecord del slot (`editorialDescription` e `inspirationSearchBrief`) en el planning store del Studio.",
    "",
    `Slot ID: ${spec.slotId}`,
    "",
    formatSlotSpecMarkdown(spec),
